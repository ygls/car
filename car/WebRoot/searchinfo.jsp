...
<%
	...
	String co=Common.toChineseAndTrim(request.getParameter("co"));
	String sql = "select * from sale where type = '"+type+"' and title like '%"+title+"%' and co like '%"+co+"%' order by id desc";
...
<TABLE class=dragTable cellSpacing=0 cellPadding=0 width="100%" border=0>
		...
		<TD  class=middle align="left" >
               <TABLE class=xsnr id=tb1_nr1  cellSpacing=0 cellPadding=0 width="100%" border=0>
                    <TBODY>
                    <% 
                    List hotlist=tb.getAllPTrave(sql);
                    if(!hotlist.isEmpty()){
        ...
        <TR>
		  ...
		  <a href="<%=basePath %>hinfo.jsp?id=<%=allnew.get(0).toString()%>"><IMG height=80 src="<%=basePath+allnew.get(3).toString()%>" width=120 onload=makesmallpic(this,120,80); border=0></A>
		   ...
		   <TD align=left><br>
		  汽车名称：<%=allnew.get(1).toString()%>  &nbsp;&nbsp;&nbsp;汽车分类：<%=allnew.get(2).toString()%><br>
		  制造商：<%=allnew.get(4).toString()%>  &nbsp;&nbsp;&nbsp;新旧程度：<%=allnew.get(5).toString()%><br>
		  会员价格：<%=allnew.get(6).toString()%>  元&nbsp;VIP价格：<%=allnew.get(7).toString()%> 元 &nbsp;&nbsp;&nbsp;购买积分：<%=allnew.get(11).toString()%> <br>
